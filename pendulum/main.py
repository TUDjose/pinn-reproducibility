...
def pinn_loss(theta, torq, t, loss_weights=[10, 1, 1], save_losses=True):
    ...
    global theta_tt
    global losses
    theta_t = grad(theta, t, grad_outputs=torch.ones_like(
        theta), create_graph=True)[0].reshape((1, -1))
    theta_tt = grad(theta_t, t, grad_outputs=torch.ones_like(
        theta_t), create_graph=True)[0].reshape((1, -1))
...
def train(model, optimizer, steps):
    ...
    def closure(time_domain, save_losses):
        optimizer.zero_grad()
        u = model(time_domain)
        theta, torq = u[:, 0], u[:, 1]
        loss = pinn_loss(theta, torq, time_domain,
                         loss_weights=loss_weights, save_losses=save_losses)
        loss.backward()
        return loss
    ...
    try:
        for n in range(steps):
            ...
            loss = closure(time_domain, save_losses=True)
            print(loss)
...
def plot_output(model):
    ...
    # Optionally plot velocity and acceleration
