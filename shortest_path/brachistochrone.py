...
class SPBPINN():
    def __init__(self):
        ...
        self.n_lbfgs = 3000
        self.n_domain = 1000
        self.lr_adam, self.lr_lbfgs = 3e-3, 3e-3
    ...
    def physics_loss(self, x, y, x_T, y_T):
        ode = self.g * self.y0 - (self.g * y + 0.5 * (x_T  ** 2 + y_T ** 2))
    ...
    def train(self, model, optimizer, steps):
        ...
        try:
            for n in range(self.n_lbfgs):
                ...
                if (loss > 1e2 and n > 100) or self.T < 0:
                    break
    ...
    def plot(self, model, show=False):
        model.eval()
        with torch.no_grad():
            u = model(self.time_domain)
            x, y = u[:, 0], u[:, 1]
...
if __name__ == "__main__":
    spb = SPBPINN()
    optimizer = torch.optim.Adam(list(spb.net.parameters()) + [spb.T], lr=spb.lr_adam)
    spb.train(spb.net, optimizer, spb.n_adam)
    spb.plot(spb.net, show=True)
